								 ...
								 var c = data.con;
									   for(i =0;i<c.length;i++)
									  {
										 // console.log('name : '+ data[i].name);
										  ...
										  {
											  htm+="<br> <button style='color : "+color+";' id='c"+ c[i].peerid+"'  onclick=coun('"+ c[i].peerid +"') disabled>"+  "CONNECT " +c[i].name   +"</button>";
											  //console.log(htm);
										  }
...
// login
 $('#login').click(function(){  // wooorks 
	...
	*/
			$.ajax({
						  url: "http://peer-peerjs.44fs.preview.openshiftapps.com/addname?name="+name+"&peer="+peerid,
						  cache: false,
						  success: function(data){
							  // var i;
...
function handleMessage(data){
  ...
  function onReceiveCall(call){
    ...
    call.on('stream', function(stream){
	  ...
	  console.log("********************************  number of connected people to you admin are  : "+remotesIDs.length);
	 ...
	 for (var i = 0; i < remotesIDs.length; i++)
	  {
		  ...
		  htm+=" <div id='peer-camera"+idd +"' class='camera' style='display : inline;'>  <video width='300' height='200' autoplay></video>  </div>";
